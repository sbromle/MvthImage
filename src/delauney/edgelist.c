...
{
	int i;
	vor_freeinit(&hfl, sizeof **ELhash);
	ELhashsize = 2 * sqrt_nsites;
	ELhash = (struct Halfedge **) vor_myalloc ( sizeof *ELhash * ELhashsize);
...
struct Halfedge *HEcreate(struct Edge *e, int pm)
{
	...
	answer = (struct Halfedge *) vor_getfree(&hfl);
	answer -> ELedge = e;
	answer -> ELpm = pm;
...
/* Hash table points to deleted half edge.  Patch as necessary. */
	...
	if ((he -> ELrefcnt -= 1) == 0) vor_makefree((struct Freenode*)he, &hfl);
